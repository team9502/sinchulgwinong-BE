...
public class FaqController {
    ...
    @Operation(summary = "FAQ 전체 조회",
            description = "FAQ 전체 목록을 페이지 기반으로 조회합니다. " +
                    "FAQ 목록에 대한 응답값은 FAQ ID, 제목, 생성일, 수정일, 조회수로 구성됩니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "FAQ 전체 조회 성공", content = @Content(
                    mediaType = "application/json",
                    examples = @ExampleObject(value = "{ \"message\": \"FAQ 전체 조회 성공\", \"data\": {\"faqs\": [{\"faqId\": 1, \"faqTitle\": \"제목\", \"createdAt\": \"2024-06-22T20:53:42.62033\", \"modifiedAt\": \"2024-06-22T20:53:42.62033\", \"viewCount\": 0}], \"totalElements\": 1, \"currentPage\": 0, \"totalPages\": 1 }}"))),
            @ApiResponse(responseCode = "500", description = "서버 에러", content = @Content(
                    mediaType = "application/json",
                    examples = @ExampleObject(value = "{ \"message\": \"서버 에러\", \"data\": null }")))
    })
    public ResponseEntity<GlobalApiResponse<FaqPageResponseDTO>> findAllFaqs(
            @PageableDefault(size = 10) Pageable pageable) {

        FaqPageResponseDTO responseDTO = faqService.findAllFaqs(pageable);
